...
const GenerateCOI_Practo: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const storedAgentData = sessionStorage.getItem('coiAgentData_Practo');
    ...
    setAgentData(data);
    sessionStorage.setItem('coiAgentData_Practo', JSON.stringify(data));
  ...
  const fetchProposalsByAgent = async (agentId: string) => {
    try {
      ...
      const response: any = await getProposalByPassport_Practo(agentId);
  ...
  const filterProposalsBySearchCriteria = (proposalsList: Proposal[], searchText: string): Proposal[] => {
    ...
    return proposalsList.filter(proposal => {
      ...
      const subId = (proposal.SubscriptionId || '').toLowerCase();

      return (
        fullName.includes(lowerFilter) ||
        policyNo.includes(lowerFilter) ||
        subId.includes(lowerFilter)
      );
    });
  };
  ...
  const goBack = () => {
    // IMPORTANT: Pass agentData state back so the Practo form can load properly
    navigate('/Practo', { state: { agentData } });
  };
        ...
        <div className="coi-header-content">
          ...
          <div className="d-flex justify-content-center py-4">
            <div className="logo d-flex align-items-center w-auto">
              <span className="page-title">Practo Subscriptions</span>
            </div>
          </div>
        ...
        <div className="coi-card">
          ...
          <div className="coi-filter-container">
            <div className="coi-filter-input-container">
              ...
              <input
                type="text"
                placeholder="Filter results (name, invoice number)"
                value={searchFilter}
                autoFocus
                ...
                <table className="coi-table">
                  <thead>
                    <tr>
                      ...
                      <th className="coi-table-header">Invoice No.</th>
                    ...
                    {currentItems.map((proposal, index) => {
                      return (
                        <tr key={index} className={index % 2 === 0 ? "coi-table-row-even" : "coi-table-row-odd"}>
                          ...
                          <td className="coi-table-cell">{proposal.proposal_id || 'N/A'}</td>
                          <td className="coi-table-cell">{proposal.FullName || 'N/A'}</td>
                          <td className="coi-table-cell">{proposal.Policy_No || 'N/A'}</td>
                          ...
                          {/* Payout */}
                          <td className="coi-table-cell">
                            {proposal.Payout_Practo ? `${proposal.Payout_Practo}%` : '0%'}
                          </td>

                          {/* Payment Mode */}
                          <td className="coi-table-cell">
                            {proposal.Payment_Mode || proposal.Selected_Payment_Mode || 'N/A'}
                          </td>
...
export default GenerateCOI_Practo;