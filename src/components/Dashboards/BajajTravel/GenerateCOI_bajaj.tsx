...
import {
  getUpdateProposalDetailsByAgent_bajaj,
  getProposalByPassport_bajaj, 
  PDF_BASE_URL,
...
const processUrl = (url: string | undefined | null, type: string): string => {
  if (!url) return '';
...
const GenerateCOI_bajaj: React.FC = () => {
  ...
  const [searchPassport, setSearchPassport] = useState<string>('');
  const [searching, setSearching] = useState<boolean>(false);
  const [searchFilter, setSearchFilter] = useState<string>('');
  const [generatingPolicies, setGeneratingPolicies] = useState<Record<string, boolean>>({});
  ...
  useEffect(() => {
    ...
    const storedAgentData = sessionStorage.getItem('coiAgentData');
    ...
    const passport = state?.passportNumber || sessionStorage.getItem('coiPassport');
  ...
  const fetchProposalsByAgent = async (agentId: string | number) => {
    try {
      setLoading(true);
      setError('');

      // Call the API with agent ID and approved status
      const response: any = await getUpdateProposalDetailsByAgent_bajaj(agentId, 'Approved');
  ...
  const fetchProposalsByPassport = async (passport: string) => {
    try {
      setLoading(true);
      setError('');

      // Call the API with the passport number
      const response: any = await getProposalByPassport_bajaj(passport);
  ...
  const handleDownload = (downloadUrl: string | undefined) => {
    if (!downloadUrl) {
  ...
  const newGeneratePolicy = async (policyNo: string) => {
    if (!policyNo) {
    ...
    try {
      ...
      const response: any = await generatePolicybyPolicyno_bajaj({
        Policyno: policyNo
      });
      ...
      const pdfUrl = response.MasterData?.combinepdfurl || 
                    (response.MasterData?.proposals && response.MasterData.proposals[0]?.PolicypdfUrl);
  ...
  const handleUpdatePolicy = (proposal: Proposal) => {
  ...
  const fetchPolicyDetails = async (policyNo: string, fallbackProposal: Proposal) => {
    try {
      setLoading(true); // Show loading state

      // Call the API to get detailed policy information
      const response: any = await generatePolicybyPolicyno_bajaj(policyNo);
  ...
  const navigateToUpdatePolicy = (proposal: Proposal) => {
    ...
    sessionStorage.setItem('proposalData', JSON.stringify({
      ...
      travelDetails: {
        ...
        numberOfDays: Math.ceil(
          (new Date(proposal.PolicyEndDate || '').getTime() - new Date(proposal.PolicyStartDate || '').getTime()) /
          (1000 * 60 * 60 * 24)
        ) || 30, // Default to 30 days if calculation fails
        geographicalCover: proposal.NameofPlan?.includes('USA') ? 'USA' : 'Worldwide',
      },
      insuranceDetails: {
        premium: parseFloat(String(proposal.Selected_PremiumAmount || proposal.PremiumAmount || 0)),
                    ...
                    {currentItems.map((proposal, index) => {
                      ...
                      return (
                        <tr key={index} className={index % 2 === 0 ? "coi-table-row-even" : "coi-table-row-odd"}>
                          ...
                          <td className="coi-table-cell">₹{parseFloat(String(proposal.PremiumAmount || proposal.Selected_PremiumAmount || 0)).toLocaleString()}</td>
                          <td className="coi-table-cell">
...
export default GenerateCOI_bajaj;