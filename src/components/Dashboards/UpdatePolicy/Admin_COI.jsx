...
import {
    getProposalDetailsByAdmin, PDF_BASE_URL
...
const Admin_COI = () => {
    ...
    useEffect(() => {
        ...
        if (data && Object.keys(data).length > 0) {
            setAgentData(data);
            sessionStorage.setItem('coiAgentData', JSON.stringify(data));
        }
        // Correctly set Admin ID from passed navigation state or storage
        const currentId = location.state?.adminId || location.state?.empid || data?.UId || '';
        if (currentId) {
            setEmpId(currentId);
        }
        ...
        fetchEmployeeProposalsWithDates(sevenDaysAgoString, todayString);

    }, [location]);

    const fetchEmployeeProposalsWithDates = async (fromDate, toDate) => {
        if (!fromDate || !toDate) {
            return;
        }
        ...
        try {
            ...
            const response = await getProposalDetailsByAdmin(fromDate, toDate);
            ...
            if (response.Status === 'Success' && response.MasterData?.proposals?.length > 0) {
                ...
                setError(`No proposals found for the specified date range`);
                setProposals([]);
                setFilteredProposals([]);
            }
        } catch (err) {
            console.error('Error auto-fetching Admin proposals:', err);
    ...
    const fetchEmployeeProposals = async () => {
        ...
        try {
            ...
            const response = await getProposalDetailsByAdmin(startDate, endDate);
            ...
            if (response.Status === 'Success' && response.MasterData?.proposals?.length > 0) {
                ...
                setError(`No proposals found for Admin ID ${empId} between ${startDate} and ${endDate}`);
                setProposals([]);
                setFilteredProposals([]);
            }
        } catch (err) {
                ...
                <div className="coi-card">
                    <div className="coi-card-header">
                        <h2 className="coi-card-title">
                            Admin Wise Proposal Details
                        </h2>
                        {empId && (
                            <p style={{ margin: '10px 0', color: '#6b7280', fontSize: '14px' }}>
                                Admin ID: <strong>{empId}</strong>
                            </p>
                        )}
                    </div>
                        ...
                        <div style={{
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                                ...
                                <input
                                    ...
                                    style={{
                                        width: '150px',
                                        backgroundColor: Boolean(location.state?.adminId || location.state?.empid || agentData?.UId) ? '#f3f4f6' : 'white'
                                    }}
                                    readOnly={Boolean(location.state?.adminId || location.state?.empid || agentData?.UId)}
                                />
                            </div>
                                ...
                                <table className="coi-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th className="coi-table-header">Policy PDF</th>

                                            <th className="coi-table-header">IndusInd Pdf</th>
                                        ...
                                        {currentItems.map((proposal, index) => {
                                            ...
                                            return (
                                                <tr key={index} className={index % 2 === 0 ? "coi-table-row-even" : "coi-table-row-odd"}>
                                                    ...
                                                    <td className="coi-table-cell">
                                                        {proposal.Policy_Url ? (
                                                            <a
                                                                ...
                                                                className="coi-action-button coi-action"
                                                            >
                                                    ...
                                                    <td className="coi-table-cell" style={{ textAlign: 'center' }}>{proposal.Payment_Mode || 'N/A'}</td>
