...
import { getProposalMIS_bajaj, PDF_BASE_URL } from "../../../services/api";
...
const MIS_Proposal_bajaj: React.FC = () => {
    ...
    const searchData = async (fromDate: string, toDate: string, employeeId: string, agentId: string): Promise<void> => {
        ...
        try {
            ...
            if (userType === 'Employee') {
                response = await getProposalMIS_bajaj(fromDate, toDate, employeeId, '');
            } else if (userType === 'Agent') {
                response = await getProposalMIS_bajaj(fromDate, toDate, '', agentId);
            } else {
                // Admin or other user types - fetch all data
                response = await getProposalMIS_bajaj(fromDate, toDate, '', '');
            }
    ...
    const gotoMIS = () => {
        ...
        navigate('/MIS_Proposal_SubAgent__ByAgent_bajaj', {
            state: {
                empid: '',
                agentData: agentData,
                userType: 'Agent',
...
export default MIS_Proposal_bajaj;