...
const MIS_Proposal: React.FC = () => {
    ...
    const searchData = async (fromDate: string, toDate: string, employeeId: string, agentId: string): Promise<void> => {
        ...
        try {
            ...
            if (userType === 'Employee') {
                response = await getProposalMIS(fromDate, toDate, employeeId, '');
            } else if (userType === 'Agent') {
                response = await getProposalMIS(fromDate, toDate, '', agentId);
            } else {
                // Admin or other user types - fetch all data
                response = await getProposalMIS(fromDate, toDate, '', '');
            }
