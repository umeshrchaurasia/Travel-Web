...
import { getSub_Main_AgentMIS_byAdmin_bajaj , PDF_BASE_URL } from "../../../services/api";
...
const MIS_Proposal_SubAgent_ByAdmin_bajaj: React.FC = () => {
  ...
  const searchData = async (fromDate: string, toDate: string, employeeId: string, agentId: string): Promise<void> => {
    ...
    try {
      ...
      const response: ApiResponse = await getSub_Main_AgentMIS_byAdmin_bajaj(fromDate, toDate, '', '');
...
export default MIS_Proposal_SubAgent_ByAdmin_bajaj