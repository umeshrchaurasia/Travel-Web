...
const MIS_Proposal_Practo: React.FC = () => {
    ...
    const searchData = async (fromDate: string, toDate: string, employeeId: string, agentId: string): Promise<void> => {
        ...
        try {
            ...
            if (userType === 'Employee') {
                response = await getProposalMIS_Practo(fromDate, toDate, employeeId, '');
            } else if (userType === 'Agent') {
                response = await getProposalMIS_Practo(fromDate, toDate, '', agentId);
            } else {
                // Admin or other user types - fetch all data
                response = await getProposalMIS_Practo(fromDate, toDate, '', '');
            }
    ...
    const downloadCSV = (): void => {
       ...
       // Generate filename with current date
        ...
        const filename = `Practo_MIS_Report_${userType}_${empId || agentId || 'All'}_${currentDate}.csv`;
                                    ...
                                    <tbody>
                                        {proposal.map((proposal: Proposal, index: number) => (
                                            <tr key={index} style={{ backgroundColor: index % 2 === 0 ? '#fff' : '#f8f9fa' }}>
                                                ...
                                                <td className="coi-table-cell">{proposal.practo_fees || ''}</td>    
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
...
export default MIS_Proposal_Practo;