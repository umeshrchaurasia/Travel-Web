...
import { getProposalTDS_bajaj, PDF_BASE_URL } from "../../../services/api";
...
const TDS_Proposal_bajaj: React.FC = () => {
   ...
   const [Main_Agent, setMain_Agent] = useState<string>(state.agentData?.Main_Agent || '');
    ...
    const searchData = async (fromDate: string, toDate: string, employeeId: string, agentId: string): Promise<void> => {
        ...
        try {
            ...
            if (userType === 'Agent') {
                response = await getProposalTDS_bajaj(fromDate, toDate, '', agentId);
            } else {
                // Admin or other user types - fetch all data
                response = await getProposalTDS_bajaj(fromDate, toDate, '', '');
            }
    ...
    const goBack = () => {
      ...
      const gotoMIS = () => {
        ...
        navigate('/TDS_Proposal_SubAgent_bajaj', {
            state: {
                empid: '',
                agentData: agentData,
                userType: 'Agent',
...
export default TDS_Proposal_bajaj;