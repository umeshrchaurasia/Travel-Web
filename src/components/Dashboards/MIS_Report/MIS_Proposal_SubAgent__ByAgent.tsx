...
import { getProposalMIS_SubAgent, PDF_BASE_URL } from "../../../services/api";
...
const MIS_Proposal_SubAgent__ByAgent: React.FC = () => {
  ...
  const searchData = async (fromDate: string, toDate: string, employeeId: string, agentId: string): Promise<void> => {
    ...
    try {
      ...
      if (userType === 'Employee') {
        response = await getProposalMIS_SubAgent(fromDate, toDate, employeeId, '');
      } else if (userType === 'Agent') {
        response = await getProposalMIS_SubAgent(fromDate, toDate, '', agentId);
      }
      else {
        // Admin or other user types - fetch all data
        response = await getProposalMIS_SubAgent(fromDate, toDate, '', '');
      }
...
export default MIS_Proposal_SubAgent__ByAgent;
